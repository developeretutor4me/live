...
export default function Home() {
  ...
  const [activeSidebarItem, setActiveSidebarItem] = useState<SidebarItemName>('Profile');
  ...
  const fetcher = useCallback(
    async (url: string): Promise<Teacher> => {
      ...
      if (!response.ok) {
        throw new Error(`HTTP ${response.status}: Teacher not found or internal server error`);
      }
  ...
  const sidebarItems = useMemo(
    (): SidebarItem[] => [
      ...
      {
        name: 'Bank Details',
        icon: activeSidebarItem === 'Bank Details' ? activebankdetails : bankdetails,
      },
      {
    ...
    (items: SidebarItem[], isFooterSection = false): JSX.Element => (
      <ul className={`space-y-2 ${isFooterSection ? 'mt-6' : 'flex-grow'}`}>
        {items.map(item => (
          <li key={item.name}>
            <button
              onClick={() => handleSidebarItemClick(item.name as SidebarItemName)}
              className={`flex hover:shadow-[0px_0px_5px_1px_rgba(255,255,255,0.3)] hover:transition-all duration-1000 items-center w-full px-6 custom-xl:px-9 py-3 sm:py-[18px] rounded-[22px] transition-all ${
  ...
  const renderContent = useCallback((): JSX.Element | null => {
    const contentMap: Record<SidebarItemName, JSX.Element | null> = {
      Profile: <Profile onFnameChange={handleFnameChange} onprofilepicture={handleProfileChange} />,
    ...
    return contentMap[activeSidebarItem] || <div>Select a tab from the sidebar</div>;
        ...
        >
          <div className="flex items-center">
            ...
            {activeSidebarItem !== 'Dashboard' && (
              <div onClick={handleBackNavigation} className="flex cursor-pointer items-center">
            ...
            >
              <div className=" flex gap-2 items-center justify-center">
                <div className="w-6 custom-xl:min-w-7 h-6 custom-xl:min-h-7  rounded-full overflow-hidden flex items-center">
                  <img src={profilepicture} alt="" className="object-cover object-center" />
                </div>
