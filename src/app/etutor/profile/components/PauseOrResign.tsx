...
function PauseOrResign({ teacher }: pauseorresignprops) {
  ...
  const submitPauseRequest = async () => {
    if (
      ...
      try {
        ...
        if (response.ok) {
          const a = await PauseTutoring.filter(
            (req: any) => req.teacher && req.teacher._id === teacher._id
          );
  ...
  const submitResignation = async () => {
    ...
    try {
      ...
      if (response.ok) {
        await setResignRequest(
          resignRequest ||
            resignTutoring.filter((req: any) => req?.teacher && req?.teacher?._id === teacher?._id)
        );
        setActiveview('Proceed');
      } else {
  ...
  useEffect(() => {
    if (teacher?._id && PauseTutoring) {
      const filteredRequests = PauseTutoring.filter(
        (req: any) => req.teacher && req.teacher._id === teacher._id
      );
  ...
  useEffect(() => {
    if (teacher?._id && resignTutoring) {
      const filteredRequests = resignTutoring.filter(
        (req: any) => req.teacher && req.teacher._id === teacher._id
      );
  ...
  if (isLoading || resignLoading) return <p>Loading...</p>;
  if (error || resignError) return <p>Error loading students: {error.message}</p>;
          ...
          {resignRequest != '' ? (
            <div className="bg-[#EDE8FA]  mt-[151px] px-14 pt-16 pb-28 rounded-3xl relative">
              ...
              <p className="text-3xl leading-snug text-[#6B5692] mb-40">
                {resignRequest && resignRequest?.filter((req: any) => req.status === 'pending') ? (
                  <span>
                    Thank you for submitting your resignation request. We’re reviewing your account
                    and will check for any ongoing commitments with students before finalizing the
                    process. Please be patient as we complete our review. We’ll reach out to you
                    shortly with further information.
                  </span>
                ) : (
                  <span>
                    We understand that circumstances can change, and sometimes you may need to step
                    back from your <br />
                    tutoring role. Whether you&apos;re looking to take a temporary break or feel
                    it&apos;s time to resign, we&apos;re here to <br />
                    support your decision. Please choose the option that aligns with your current
                    needs.
                  </span>
                )}
              </p>
            ...
            <div className="bg-[#EDE8FA]  mt-[151px] px-14 pt-16 pb-28 rounded-3xl relative">
              <h1 className="text-xl sm:text-2xl custom-xl:text-6xl text-[#685AAD] font-bold  mb-9 ">
                ...
                filterRequest?.filter((req: any) => req.status === 'pending')
                  ? 'Request Pending'
                  : 'Pause or Resign'}
              </h1>
              <p className="text-3xl leading-snug text-[#6B5692] mb-40">
                ...
                filterRequest?.filter((req: any) => req.status === 'pending') ? (
                  <span>
                    Thank you for submitting your request to pause your tutoring sessions. We’re
                    currently reviewing your account, including any ongoing plans with students and
                    other relevant details. Please be patient as we complete this process. We’ll get
        ...
        <div className="bg-white absolute top-0 left-0 z-50 min-h-screen w-screen ">
          <div className="flex flex-col my-16 gap-10 ">
            ...
            <div className="bg-[#EDE8FA]   pl-24 pr-16  pt-16 pb-5 rounded-3xl relative max-w-[91rem]  w-full mx-auto">
              ...
              <p className="text-3xl leading-snug text-[#6B5692] mb-40">
                Need some time off? By pausing your tutoring sessions, you can take a break without
                losing your eTutor <br />
                profile, and you can easily resume tutoring when you&apos;re ready. This option is
                perfect if you plan to return in the <br />
                near future but need some time away for personal, academic, or professional reasons.
              </p>
            ...
            <div className="bg-[#EDE8FA]   pl-24 pr-16  pt-16 pb-5 rounded-3xl relative max-w-[91rem] w-full  mx-auto">
              ...
              <p className="text-3xl leading-snug text-[#6B5692] mb-20">
                If you’ve decided to step away from tutoring permanently, you can choose to resign.
                Resigning will <br />
                deactivate your account, and you will no longer be matched with students. We greatly
                value the impact <br />
                you’ve made and will always welcome you back should you decide to return in the
                future. We encourage you <br />
                to complete a brief exit form so we can better understand your experience and
                continue improving our <br /> platform.
              </p>
        ...
        <div className="bg-white absolute top-0 left-0 z-50 min-h-screen w-screen py-[84px] px-[125px]">
          <div className="bg-[#EDE8FA]   px-24 py-20 rounded-3xl relative   w-full mx-auto">
            ...
            <p className="text-3xl leading-snug text-[#6B5692] mb-28">
              Before you submit your request to pause tutoring, please complete the following form.
              Your input is valuable and will <br />
            ...
            <div className="w-full max-w-[35.5rem] mt-11">
              ...
              <div className="relative  select-none mt-6 ">
                ...
                >
                  <span className="my-1">
                    {selectedReasonToPauses != '' ? `${selectedReasonToPauses}` : 'select a reason'}
                  </span>
            ...
            <div className="mt-20 w-full max-w-[54rem]">
              <label className="block text-lg sm:text-xl font-semibold text-[#685AAD]">
                Specify the length of time you plan to pause (e.g., 1 month, 3 months){' '}
      ...
      {activeview === 'resign' && (
        <div className="bg-white absolute top-0 left-0 z-50 min-h-screen w-screen py-[84px] px-[125px]">
          <div className="bg-[#EDE8FA]   px-24 py-20 rounded-3xl relative   w-full mx-auto">
            ...
            <p className="text-3xl leading-snug text-[#6B5692] mb-28">
              Before you submit your resignation, please complete the following form. Your feedback
              will help us improve our <br />
