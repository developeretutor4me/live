...
const TutoringDashboard = ({ teacher }: earningprops) => {
  ...
  useEffect(() => {
    ...
    if (teacher?.TotalGroupSession > 0) {
      ...
      setTotalregularsession(
        Math.min(
          (teacher?.TotalRegularSession /
            (teacher?.TotalRegularSession + teacher?.TotalGroupSession)) *
            100,
          90
        )
      );
  ...
  useEffect(() => {
    ...
    setCurrentmonthperc(
      Math.min(
        (teacher?.currentMonthRegularSession /
          (teacher?.currentMonthRegularSession + teacher?.currentMonthGroupSession)) *
          100,
        90
      )
    );
  ...
  return (
    <div
      className={`w-full  mx-auto   ${
        (activeTab === 'Online tutoring' || activeTab === 'Earnings PDFs') && 'mt-24'
      } ${
    ...
    >
      ...
      {activeTab === 'Online tutoring' && (
        <div className="bg-[#B4A5D7] rounded-[33px] px-11 pb-[34px] pt-[30px] mb-11">
          <h2 className="text-white text-[30px] leading-[2.25rem] font-semibold mb-4">
            Your tax information is missing. Fill it in to guarantee regular payouts.
          </h2>
          <p className="text-white text-[22px] leading-[1.6rem] font-normal mb-3">
            As a legal requirement, we need to collect your tax information. If we don&apos;t
            receive it, we will suspend your payouts and account until you provide it. <br />
            Fill the tax information section of your Profile as soon as possible to keep your
            payouts regular.
          </p>
          <a href="#" className=" text-[22px] leading-[1.6rem] font-bold text-[#685AAD] ">
            <span
              ...
              className="underline text-[#9052FC]"
            >
              {' '}
              Click here
            </span>{' '}
            to Fill in tax information now
          </a>
        </div>
      )}
      {activeTab === 'Earnings PDFs' && (
        <div className="bg-[#B4A5D7] rounded-[33px] px-11 py-[30px] mb-10">
          ...
          <a href="#" className=" text-[22px] leading-[1.75rem] font-bold text-[#685AAD]">
            <span className="underline text-[#9052FC]"> Click here</span> to Fill in tax information
            now
          </a>
        </div>
      )}
      ...
      {/* Main Dashboard */}
      <div
        className={`bg-[#EDE8FA] rounded-[35px]  ${
      ...
      >
        {/* Custom Tabs */}
        <div
          className={`flex   justify-between   text-xl  scrollbar-none overflow-x-scroll w-full gap-12 ${
            (activeTab === 'Previous sessions' || activeTab === 'Earnings PDFs') && 'px-7 '
          } `}
        >
          {tabs.map(tab => (
            <button
              key={tab}
              className={`pb-0.5 text-nowrap ${
                activeTab === tab
          ...
          <div className="flex justify-between flex-wrap gap-8 md:gap-14 mt-8 md:mt-16 pb-6 md:pb-9 px-4 md:px-0">
            <div className="flex flex-col w-full sm:w-[22.5rem] py-2 text-[#685AAD]">
                  ...
                  {erroretokies
                    ...
                    : (isLoadingetokies
                        ...
                        : etokies
                            ?.filter(
                              (etokie: any) =>
                                new Date(etokie.createdAt).getMonth() === new Date().getMonth() &&
                                new Date(etokie.createdAt).getFullYear() ===
                                  new Date().getFullYear()
                            )
                            .reduce((sum: any, etokie: any) => sum + etokie.amount, 0)) || 0}{' '}
                  eTokis
                </span>
              </div>
              ...
              <div className="bg-[#9052FC] w-full h-10 md:h-12 rounded-lg mt-3">
                <div
                  style={{ width: `${currentmonthperc}%` }}
                  className={`h-full  bg-[#FF7777] rounded-lg`}
                ></div>
              </div>
            ...
            <div className="flex flex-col w-full sm:w-[22.5rem] py-2 text-[#685AAD]">
              ...
              <div className="bg-[#9052FC] w-full h-10 md:h-12 rounded-lg mt-3">
                <div
                  style={{ width: `${Totalregularsession}%` }}
                  className="h-full  bg-[#FF7777] rounded-lg"
                ></div>
              </div>
            ...
            <div className="flex justify-between w-full sm:w-[22.5rem] p-4 md:p-6 bg-[#B4A5D7] text-white text-sm md:text-base rounded-xl">
              <div className="flex flex-col justify-between w-full gap-3 md:gap-4">
                <h1 className="text-[#685AAD] font-bold text-lg md:text-2xl">Tutoring overview</h1>
          ...
          <div className="mt-12 md:mt-20">
            <div className="py-2 text-[#685AAD] px-4 md:px-0">
              ...
              <p className="text-lg md:text-[22px] md:leading-[1.75rem]">
                Our eTokis programs are a way to say thank you for being a part of our eTutor
                community and for your constant commitment to helping us unlock
                <span className="hidden md:inline">
                  {' '}
                  <br />
                </span>
                the full potential of every child. through these programs you have the opportunity
                to access lump sum eTokis and session rate increases.
              </p>
            </div>
            ...
            <div className="w-full mt-8 md:mt-14 px-4 md:px-10 py-6 md:pt-[25px] md:pb-[30px] bg-[#B4A5D7] rounded-xl flex justify-between items-center flex-wrap gap-4">
              ...
              <div className="text-white w-full md:w-[87.7%] flex flex-col md:flex-row md:items-end md:justify-between gap-3 ">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold mb-3">Loyalty eTokis</h1>
                  <p className="text-lg md:text-[22px] md:leading-[1.75rem]">
                    Complete a minimum of 150 sessions in six months to increase your lesson rate.
                  </p>
                </div>
            ...
            <div className="w-full mt-4 md:mt-8 mb-8 md:mb-16 px-4 md:px-10 py-6 md:py-[30px] bg-[#B4A5D7] rounded-xl flex justify-between items-center flex-wrap gap-4">
              ...
              <div className="text-white w-full md:w-[87.7%] flex flex-col md:flex-row md:items-end md:justify-between gap-4">
                <div>
                  <h1 className="text-2xl md:text-3xl font-bold mb-3">Referral bonus</h1>
                  <p className="text-lg md:text-[22px] md:leading-[1.75rem]">
                    Share your referral link and receive a bonus for every new tutor or student who
                    joins
                    <span className="hidden md:inline">
                      {' '}
                      <br />
        ...
        {activeTab === 'Additional information' && (
          <div>
            <div className="mt-[68px] text-[#685AAD]">
              <h1 className="text-[45px] leading-[2.25rem] mb-8 font-bold">
                Regular session rates
              </h1>
              <p className="text-[22px] leading-[1.75rem] ">
                Here are the rates for each subject you’re qualified to tutor in. to view your exact
                payments for <br />
