...
function EtutorsList({ filteredTutors, handleViewProfile, handleBackToSearch }: any) {
  const [filteredTutorsList, setFilteredTutorsList] = useState<any>(filteredTutors || []);
  ...
  const handleOptionClick = (value: string) => {
    let filteredTeachers = [...filteredTutorsList];
    ...
    setFilteredTutorsList(filteredTeachers);
  };
  ...
  const onSearchClick = (searchText: string) => {
    ...
    if (searchText && typeof searchText === 'string' && searchText.trim()) {
      let searchTerm = searchText.toLowerCase().trim();
  ...
  return (
    <div className="bg-[#EDE8FA] w-full rounded-3xl px-8 py-6">
      ...
      <div className="flex flex-col gap-4 custom-lg:gap-9">
        {filteredTutorsList?.length > 0 &&
          filteredTutorsList?.map((teacher: any) => (
            <div
              // @ts-ignore
              key={teacher?._id}
              ...
              <div className="  h-fit  w-full custom-xl:max-w-[20rem] ">
                ...
                <div className=" flex flex-col items-center sm:items-start">
                  <div className="mt-2 flex flex-col items-center sm:items-start">
                    ...
                    <div className="text-lg text-[#473171] text-center sm:text-start flex">
                      {Object.keys(teacher?.experience?.generalAvailability || {})
                        .slice(0, 3)
                        .map((day, index, array) => (
                          <span key={day} className="flex flex-row">
                            <span className="flex">
                              {day}
                  ...
                  <div className="mt-4 flex flex-col items-center sm:items-start">
                    ...
                    <p className="text-lg text-[#473171] text-center sm:text-start">
                      ...
                      teacher?.experience?.subjectsTutored.map((subject: any) => subject).join(',')}
                    </p>
                  </div>
                </div>
              </div>
