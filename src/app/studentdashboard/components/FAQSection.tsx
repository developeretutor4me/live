...
const FAQSection = ({ onNeedMoreHelp, onChatHistory }: any) => {
  ...
  return (
    <div className={`flex ${!isVisible && 'flex-col'} gap-4 custom-xl:h-full scrollbar-none`}>
      {/* faq box */}
      <div
        className={`${isVisible ? 'max-w-[957px]' : ''}  order-2 custom-xl:max-h-[927px] h-fit custom-xl:h-full rounded-2xl sm:rounded-3xl custom-xl:rounded-[30px] px-4 sm:px-10 custom-xl:px-[78px] py-4 sm:py-10 custom-xl:py-16 sm:mt-5 w-full  flex flex-col custom-xl:flex-row bg-[#EDE8FA]`}
      >
      ...
      {/* button box */}
      <div
        className={`${isVisible && 'absolute right-0'} order-1  flex justify-end flex-col items-end`}
      >
        <div
          className={`${!isVisible && 'hidden'} bg-[#EDE8FA] rounded-l-2xl px-12 py-12 mt-[84px]  w-[486.59px] h-[227.25px] text-lg sm:text-2xl custom-xl:text-[52.67px] flex  flex-col justify-center   font-bold text-[#685AAD] `}
        >
