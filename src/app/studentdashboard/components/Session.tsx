...
const SessionDashboard = ({ studentdata }: sessionprops) => {
  ...
  useEffect(() => {
    ...
    const fetchRequests = async () => {
      try {
        setLoading(true);
        const response = await fetch(`/api/Fetch-sendingrequests-fromteacher?userId=${userId}`);
        if (!response.ok) {
      ...
      <div className={`${styles.maindiv} `}>
        ...
        <div className={`${styles.contentdiv}   `}>
          <div className="relative  h-full ">
            <div
              className={`${
                activeTab === 'trial' ? 'hidden' : 'hidden custom-2xl:flex'
