...
function WebsiteVisits() {
  ...
  useEffect(() => {
    if (googleAnalytics?.dailyData) {
      const processAnalyticsData = () => {
        ...
        // Daily visits
        const dailyVisits =
          dailyData.find((d: { date: string }) => d.date === currentDateStr)?.sessions || 0;
        ...
        const yearlyVisits = dailyData
          .filter(
            (d: { date: string | number | Date }) => new Date(d.date).getFullYear() === currentYear
          )
          .reduce((sum: any, day: { sessions: any }) => sum + day.sessions, 0);
        ...
        const previousDayVisits =
          dailyData.find((d: { date: string }) => d.date === prevDayStr)?.sessions || 0;
        ...
        switch (currentName) {
          case 'Daily':
            percentageChange = previousDayVisits
              ? ((dailyVisits - previousDayVisits) / previousDayVisits) * 100
              : 0;
            break;
          case 'Weekly':
            percentageChange = previousWeekVisits
              ? ((weeklyVisits - previousWeekVisits) / previousWeekVisits) * 100
              : 0;
            break;
          case 'Monthly':
            percentageChange = previousMonthVisits
              ? ((monthlyVisits - previousMonthVisits) / previousMonthVisits) * 100
              : 0;
            break;
          case 'Yearly':
            percentageChange = previousYearVisits
              ? ((yearlyVisits - previousYearVisits) / previousYearVisits) * 100
              : 0;
            break;
        }
  ...
  const [storedWeeks, setStoredWeeks] = useState([getFormattedWeek(new Date())]);
      ...
      <div className="flex justify-between h-full ">
        <div className="flex flex-col justify-between  h-full px-3 py-1.5 ">
          ...
          <h1 className="text-base sm:text-lg custom-lg:text-2xl font-bold leading-none text-[#a398cf] flex items-center  gap-3">
            {' '}
            <ChevronDown
              className={`font-extrabold ${visitsData.percentage >= 0 ? 'rotate-180' : ''}`}
            />
            {Math.abs(visitsData.percentage).toFixed(1)}%
          </h1>
        </div>
