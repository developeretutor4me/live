  ...
  return (
    <div className=" w-full h-fit flex flex-col custom-2xl:flex-row">
      <div className="  w-full h-full">
        <h2 className="text-lg sm:text-2xl custom-2xl:text-4xl font-bold text-[#685AAD] mb-0 ml-4">
        ...
        <ul className="mt-4 px-2 custom-2xl:px-9  custom-2xl:mt-10 ">
          {faqs.map((faq, index) => (
            <li
              key={index}
              className="py-4 custom-2xl:py-8 text-xs sm:text-lg custom-2xl:text-2xl border-b text-[#534988]  border-[#ada7cfc4] last:border-b-0  "
            >
              <button
              ...
              >
                <Image
                  ...
                  src={downarrow}
                  alt=""
                  className={` sm:h-4 custom-2xl:h-7  h-2 w-2 sm:w-4 custom-2xl:w-7 text-black font-thin transition-transform ${openIndex === index ? 'transform rotate-180' : ''}`}
                />
