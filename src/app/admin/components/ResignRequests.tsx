...
function ResignRequests() {
  ...
  if (resignLoading) return <p>Loading...</p>;
  if (resignError) return <p>Error loading students: {resignError.message}</p>;
  ...
  return (
    <div className="mt-10 bg-[#ede8fa] rounded-md sm:rounded-xl  custom-lg:rounded-3xl h-fit  px-3 custom-xl:px-10  py-3 custom-xl:py-10  relative">
      ...
      <section className="  mt-3 custom-xl:mt-12  leading-none  overflow-hidden">
        ...
        >
          {resignTutoring
            .filter((resign: any) => resign.status === 'pending')
            .map((resign: any, index: any) => (
              <div
                key={index}
