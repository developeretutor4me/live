...
function PaidSessions() {
  ...
  const filteredBookings = booking.filter((booking: any) => {
    ...
    const teacherName = booking.teacher?.contactInformation?.firstName?.toLowerCase() || '';
  ...
  return (
    <div className="mt-10 bg-[#ede8fa] rounded-md sm:rounded-xl  custom-lg:rounded-3xl h-fit  px-3 custom-xl:px-10  py-3 custom-xl:py-10  relative">
      ...
      <section className="  mt-3 custom-xl:mt-8  leading-none  overflow-hidden">
        ...
        >
          {sortedBookings
            .filter((booking: any) => booking.IsTrialSession === false)
              ...
              >
                <div className="h-[60px] sm:h-[107px] w-full rounded-md sm:rounded-xl  custom-lg:rounded-3xl flex items-center justify-between custom-xl:justify-normal  gap-2 sm:gap-5 px-4 custom-lg:px-14 ">
                  ...
                  <div className=" gap-4 items-center w-[14rem] hidden custom-lg:flex">
                    <div
                      className={`${booking.status === 'pending' ? 'bg-[#00dae5]' : booking.status === 'accepted' && booking.meetingCompleted === true ? 'bg-[#7a6cb7]' : booking.status === 'rejected' ? 'bg-[#fc7777]' : booking.status === 'accepted' ? 'bg-[#8653ff]' : ''} h-[25px] w-[25px] rounded-sm`}
                    >
                      &nbsp;
                    </div>
                    <h1 className="text-white  text-3xl font-medium">
                      {booking.status === 'pending'
                        ? 'Unconfirmed'
                        : booking.status === 'accepted' && booking.meetingCompleted === true
                          ? 'Completed'
                  ...
                  <div className="block custom-xl:hidden text-white">
                    <button
                      onClick={() => {
                        setHover(prevHover => (prevHover === booking._id ? null : booking._id));
                      }}
                    >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 md:gap-y-4 md:grid-cols-3  w-[85%] mx-auto my-3 custom-xl:hidden">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2  ">
                      <div
                        className={`${booking.status === 'pending' ? 'bg-[#00dae5]' : booking.status === 'accepted' && booking.meetingCompleted === true ? 'bg-[#7a6cb7]' : booking.status === 'rejected' ? 'bg-[#fc7777]' : booking.status === 'accepted' ? 'bg-[#8653ff]' : ''}   h-3 custom-xl:h-4 w-3 custom-xl:w-4 rounded custom-xl:rounded-sm`}
                      >
                        &nbsp;
                      </div>
                      <h1 className="text-white  text-xs sm:text-sm custom-lg:text-lg custom-xl:text-xl font-medium">
      ...
      {ispopupopen && activetab === '' && (
        <div className="popup  h-[29.5rem] rounded-3xl  py-8 px-10 w-[40.25rem] bg-[#ede8fa] z-50 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <h1 className="text-[#fb8384] text-[40px] leading-none font-bold">Select an Action</h1>
          <p className="text-[#7669b5] text-lg mt-5 leading-tight">
            Choose the action you want to take for this transaction. You will need to <br /> enter
            your admin passkay to proceed
          </p>
      ...
      {ispopupopen && activetab === 'comment' && (
        <div className="popup w-[46.75rem] min-h-[28.4rem] rounded-3xl  py-8 px-10  bg-[#ede8fa] z-50 absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <div className="flex justify-between w-full">
            <h1 className="text-[#fb8384] text-[40px] leading-none font-bold">Leave a Comment</h1>
            <Image
              loading="lazy"
              onClick={() => {
          ...
          <p className="text-[#7669b5] text-lg mt-5 leading-tight">
            Provide internal notes or feedback about this transaction. These comments <br /> will be
            recorded and visible to other admins for future reference.
          </p>
