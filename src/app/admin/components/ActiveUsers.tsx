...
function ActiveUsers() {
  ...
  const activeUsers = googleAnalytics?.realtime?.activeUsers || 0;
  const dailyData = googleAnalytics?.dailyData || [];

  // Find today's peak users
  const today = new Date().toISOString().split('T')[0];
  const todayData = dailyData.filter((data: any) => data.date === today);
  const todaysPeak = todayData.length
    ? Math.max(...todayData.map((data: any) => data.activeUsers))
    : 0;
        ...
        <div className="flex  justify-between items-center  ">
          <div className="flex flex-col  justify-between   px-3 py-1.5">
            <h1 className="flex gap-5 items-center text-xl sm:text-3xl custom-lg:text-[43px] leading-10 text-[#7669b5] font-medium py-4">
              <div className="bg-[#00dae5] h-[25px] w-[25px] rounded-sm">&nbsp;</div> Active users
            </h1>
