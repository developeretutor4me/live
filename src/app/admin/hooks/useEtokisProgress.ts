...
import { LEVELS, Level, ProgressResult } from '@/app/etutor/profile/components/Data';

const getCurrentLevel = (etokis: number): Level => {
  if (etokis < 0) throw new Error(`ETokis cannot be negative: ${etokis}`);
  return [...LEVELS].reverse().find(lvl => etokis >= lvl.etokisRequired) || LEVELS[0];
};
...
const calculateProgress = (etokis: number, current: Level, next: Level | null): number => {
  if (!next) return 100;
