...
const MembershipPayment = ({
  ...
  const [subtotalText, setSubtotalText] = useState(
    `Pay in Full (15%): $${(totalAmount * 9 - (15 / 100) * (totalAmount * 9)).toFixed(2)}`
  );
  ...
  const handleOptionClick = (option: any) => {
    ...
    if (option === 'full') {
      ...
      setSubtotalText(
        `Every 3 Months (10%): $${(totalAmount * 9 - (10 / 100) * (totalAmount * 9)).toFixed(2)}`
      );
    } else {
      // setTotalAmount("$232.00/m");
      setSubtotalText(
        `Monthly Payment (0%) $${(totalAmount * 9 - (0 / 100) * (totalAmount * 9)).toFixed(2)}`
      );
    }
  };
  ...
  return (
    <div className="w-full  max-w-[1196px] mx-auto bg-white rounded-3xl drop-shadow-[0px_0px_10px_rgb(0,0,0,0.7)] p-8 md:p-12 custom-xl:px-16 custom-xl:py-12 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50">
      ...
      <p className="text-[#776bb5] custom-xl:text-[21.86px] custom-xl:leading-[1.75rem]  mb-8 custom-xl:mb-9">
        Choose your preferred payment schedule for your 9-month Standard Membership with a Junior
        eTutor.
        <br />
      ...
      <div className="flex flex-col md:flex-row gap-8 custom-xl:gap-16">
        <div className="flex-1 flex flex-col gap-6 custom-xl:mt-4">
          ...
          >
            ...
            {(hoveredOption === 'full' || selectedOption === 'full') && (
              <p className="text-[#776bb5] mt-2">
                Pay the entire amount upfront and enjoy a 15% discount on the total price.
              </p>
            )}
          </button>
          ...
          >
            <div className="flex justify-between items-center">
              <span className="text-xl custom-xl:text-3xl text-[#776bb5]">Pay Every 3 Months</span>
              <span className="text-xl custom-xl:text-[27.49px] custom-xl:leading-[2rem] font-semibold text-[#776bb5]">
                10% discount
              </span>
            </div>
            {(hoveredOption === 'quarterly' || selectedOption === 'quarterly') && (
              <p className="text-[#776bb5] mt-2">
                Split your payment into three equal installments, billed every 3 months, and enjoy a
          ...
          >
            <div className="flex justify-between items-center">
              <span className="text-xl custom-xl:text-3xl text-[#776bb5] ">Pay Monthly</span>
            </div>
            {(hoveredOption === 'monthly' || selectedOption === 'monthly') && (
              <p className="text-[#776bb5] mt-2">
                Pay on a monthly basis with automatic renewal. This option does not include any
                discounts.
              </p>
            )}
          </button>
        ...
        <div className="md:w-[363px]  custom-xl:mr-3 custom-xl:mt-1">
          <div className="border  rounded-3xl custom-xl:rounded-[45px] p-6 custom-xl:px-14 custom-xl:py-10 mb-8 custom-xl:mb-10 border-[#685AAD]">
            <div className="mb-4">
              ...
              <p className="text-[#8653ff] custom-xl:text-2xl font-medium">9 months</p>
            </div>
            ...
            <div className="border-t border-[#c2a9ff] pt-5">
              <div className="flex justify-between items-center mb-2">
                ...
                <span className="text-3xl text-[#8653FF] font-medium">${totalAmount}/m</span>
              </div>
              <p className="text-[#867bbd] font-medium text-xl">{subtotalText}</p>
            </div>
          </div>
