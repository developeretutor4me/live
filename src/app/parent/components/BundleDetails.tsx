...
const BundlesComponent = ({ tutorLevel, bundles_session, priceData }: BundlesComponentprops) => {
          ...
          {bundles.map(bundle => (
            ...
            >
              <h2
                ...
                ${selectedCard === bundle.id ? 'text-[#8653FF] ' : 'text-[#685AAD]'}
                `}
              >
                {bundle.title}
              </h2>
              ...
              <div className="flex sm:justify-between sm:items-center flex-col sm:flex-row">
                <span className="text-xl font-bold text-[#8653FF]">${bundle.price}</span>
