  ...
  >
    <div className="flex items-center">
      ...
      <div className="flex-grow ">
        <p className={`font-bold text-[16px] md:text-[22px] truncate`}>
          {assignedTo || 'Admin Assistant'}
        </p>
        <p className="hidden md:block text-[12px] text-[#FFFFFF]/60">{tagline}</p>
      </div>
    </div>
  </div>
);

const ChatMessage = ({ message, isUser, ticketAssignedTo }: any) => {
  if (!message || !message.message || !message.createdAt) return null;

  return (
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'} mb-2 custom-2xl:mb-4 w-full`}>
      <div
      ...
      >
        {!isUser && (
          <p className="text-sm sm:text-base custom-2xl:text-xl font-bold transition-all text-[#685aad]">
            {ticketAssignedTo || 'Admin Assistant'}{' '}
            <span className="text-xs text-[#b3acd6]">/ Assistant</span>
          </p>
        )}
  ...
  return (
    <div className="w-full h-full">
      <div className="flex h-full gap-3 custom-2xl:gap-6 overflow-hidden">
        {/* Sidebar */}
        <div className="hidden sm:flex sm:flex-col w-[24.4%] bg-[#EDE8FA] h-full overflow-hidden px-1">
          <div className="pt-3 w-fit">
            <h2 className="font-bold text-[#685AAD] text-[25px] sm:[30px] md:text-[35px] lg:text-[45px] custom-xl:text-[54px]">
              In this chat
            </h2>
          </div>
          <div className="hidden pt-7 overflow-y-auto scrollbar-thin sm:flex flex-col gap-3 custom-2xl:gap-6 scrollbar-track-transparent scrollbar-thumb-[#685aad40] scrollbar-thumb-rounded-3xl flex-1 min-h-0">
        ...
        <div className="flex-grow w-[75.5%] flex flex-col rounded-xl sm:rounded-2xl custom-xl:rounded-3xl bg-[#A296CC] h-full max-w-full">
          {/* Chat Header */}
          <div className="bg-transparent py-3 custom-2xl:py-5  px-4 flex items-center rounded-t-3xl  pl-6 custom-2xl:pl-10 border-b-2 border-[#8C55FF]/20 mx-5">
            <Image
              loading="lazy"
              src={chaticon}
              alt="Chat Icon"
              className="mr-3 custom-2xl:mr-5 w-5 custom-2xl:w-8 h-5 custom-2xl:h-8"
            />
            <h2 className="text-xl custom-2xl:text-3xl font-bold text-white">Support Chat</h2>
          </div>

          {/* Messages */}
          <div className="flex-1 min-h-0 p-1 custom-2xl:p-3 bg-[#A296CC] mx-4 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-[#685aad40] scrollbar-thumb-rounded-3xl">
            {loading ? (
          ...
          {ticket.status === 'active' ? (
            <div className="flex-shrink-0 py-2 sm:py-4 px-2 sm:px-10 bg-[#A296CC] rounded-b-3xl">
            ...
            <div className="flex-shrink-0 px-2 py-6 bg-[#A296CC] rounded-b-3xl">
              <p className="bg-[#b4a5d7] max-w-[95%] mx-auto text-xs sm:text-lg custom-lg:text-2xl font-medium text-center py-4 sm:py-7 px-4 sm:px-10 xl:px-20 rounded-2xl xl:rounded-3xl leading-tight">
