...
export default function PlanDetails({ plan, onBack, priceData }: any) {
  ...
  const getPlanPrice = (planType: string, tutorLevel: string, month: string) => {
    return priceData?.data?.[planType]
      ?.filter(
        (plan: any) =>
          plan.planType === planType && plan.tutorLevel === tutorLevel && plan.month === month
      )
  ...
  const planData = {
    premium: {
      ...
      sessions: 8,
      price:
        selectedDuration === '3 months' && selectedLevel === 'Junior'
          ? getPlanPrice('premium', 'Junior', '3_Months')
          : selectedDuration === '3 months' && selectedLevel === 'Senior'
            ? getPlanPrice('premium', 'Senior', '3_Months')
            : selectedDuration === '3 months' && selectedLevel === 'Expert'
              ? getPlanPrice('premium', 'Expert', '3_Months')
              : selectedDuration === '6 months' && selectedLevel === 'Junior'
                ? getPlanPrice('premium', 'Junior', '6_Months')
                : selectedDuration === '6 months' && selectedLevel === 'Senior'
                  ? getPlanPrice('premium', 'Senior', '6_Months')
                  : selectedDuration === '6 months' && selectedLevel === 'Expert'
                    ? getPlanPrice('premium', 'Expert', '6_Months')
                    : selectedDuration === '12 months' && selectedLevel === 'Junior'
                      ? getPlanPrice('premium', 'Junior', '12_Months')
                      : selectedDuration === '12 months' && selectedLevel === 'Senior'
                        ? getPlanPrice('premium', 'Senior', '12_Months')
                        : selectedDuration === '12 months' && selectedLevel === 'Expert'
                          ? getPlanPrice('premium', 'Expert', '12_Months')
                          : '',
      bgColor: 'bg-[#5553C4]',
    },
    standard: {
      ...
      price:
        selectedDuration === '3 months' && selectedLevel === 'Junior'
          ? getPlanPrice('standard', 'Junior', '3_Months')
          : selectedDuration === '3 months' && selectedLevel === 'Senior'
            ? getPlanPrice('standard', 'Senior', '3_Months')
            : selectedDuration === '3 months' && selectedLevel === 'Expert'
              ? getPlanPrice('standard', 'Expert', '3_Months')
              : selectedDuration === '6 months' && selectedLevel === 'Junior'
                ? getPlanPrice('standard', 'Junior', '6_Months') / 4
                : selectedDuration === '6 months' && selectedLevel === 'Senior'
                  ? getPlanPrice('standard', 'Senior', '6_Months') / 4
                  : selectedDuration === '6 months' && selectedLevel === 'Expert'
                    ? getPlanPrice('standard', 'Expert', '6_Months') / 4
                    : selectedDuration === '12 months' && selectedLevel === 'Junior'
                      ? getPlanPrice('standard', 'Junior', '12_Months') / 9
                      : selectedDuration === '12 months' && selectedLevel === 'Senior'
                        ? getPlanPrice('standard', 'Senior', '12_Months') / 9
                        : selectedDuration === '12 months' && selectedLevel === 'Expert'
                          ? getPlanPrice('standard', 'Expert', '12_Months') / 9
                          : '00.00',
      bgColor: 'bg-[#53497F]',
    },
    bundles: {
    ...
    <>
      <div className="bg-white rounded-3xl custom-xl:rounded-[45px]  overflow-hidden border border-black   drop-shadow-[0px_5px_0px_#b0a9c4] sm:drop-shadow-[0px_15px_0px_#b0a9c4] custom-lg:drop-shadow-[0px_20px_0px_#b0a9c4] ">
        ...
        <div className="px-4 sm:px-10 custom-xl:px-12  py-3 sm:py-6 custom-xl:py-8 mb-4">
          {plan != 'bundles' ? (
            <>
              <div className="flex w-full mx-auto items-start custom-xl:items-center flex-col custom-xl:flex-row custom-xl:justify-between custom-xl:flex-wrap">
                <div className="flex flex-col items-center justify-center border-b-2 custom-xl:border-b-0 custom-xl:border-r-2 border-[#e0d4ff] custom-xl:max-w-[21.2rem] pb-4 custom-xl:pb-0 mb-4 custom-xl:mb-0 pr-0 custom-xl:pr-6">
                  <h2 className="text-[#9C78F9] text-left font-bold sm:text-[16px] md:text-[24px] lg:text-[32px] xl:text-[45px] 2xl:text-[50px]">
                    Choose your Preferences
                  </h2>
                </div>

                <div className="flex flex-col items-start justify-center gap-y-0 lgsm:gap-y-4 custom-xl:gap-y-8 border-b-2 custom-xl:border-b-0 custom-xl:border-r-2 border-[#e0d4ff] pb-4 custom-xl:pb-0 mb-4 custom-xl:mb-0 pr-0 custom-xl:pr-6">
                  <div className="text-[#8653ff] text-lg  custom-xl:text-2xl custom-xl:text-[54.25px] custom-xl:leading-[2.25rem] font-medium ">
                    ...
                    <span className=" text-sm sm:text-xl custom-xl:text-4xl"> / month</span>
                  </div>
                    ...
                    <div className="flex flex-wrap gap-3 mt-0.5 pl-1">
                      {['3 months', '6 months', '12 months'].map(duration => (
                        <button
                          ...
                          } py-2 sm:py-3 px-[55px] rounded-full text-sm sm:text-xl custom-xl:text-[32.98px] custom-xl:leading-[2.25rem]  transition-all   `}
                        >
                          {duration}
                        </button>
                      ))}
            ...
            <div className="">
              <BundleDetails
                ...
                bundles_session={setBundlessession}
                priceData={priceData}
              />
            </div>
          )}
