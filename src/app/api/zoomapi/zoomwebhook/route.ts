  ...
  if (headers.get('x-zm-signature') === signature) {
    ...
    // Handle meeting ended event
      ...
      try {
        ...
        const updatedBooking: any = await BookingModel.findOneAndUpdate(
          { 'zoomMeetingData.meeting.id': meetingId },
          { meetingCompleted: true },
          { new: true }
        );
        const TrialSession: any = await BookingModel.findOne({
          'zoomMeetingData.meeting.id': meetingId,
        });
        ...
        await UserModel.findByIdAndUpdate(meetingUser, { $inc: { etokis: 10 } }, { new: true });
